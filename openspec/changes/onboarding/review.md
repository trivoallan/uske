...
| --- | --- | --- | --- | --- |
| 1 | Ingénierie · vérification | La commande prescrite par chaque ADR, `npx @jackchuka/mdschema@0.15.1 check '*.md'`, échoue sur ce poste : `sh: mdschema: command not found`. Cause d'abord supposée : un cache périmé — **faux**, la purge n'a rien changé. Cause établie à l'application : le `npm` 10.9.8 de `~/.local/bin` ne crée pas le lien binaire de ce paquet ; le `npm` 11.19.0 le crée. Le binaire, lancé par `node …/@jackchuka/mdschema/bin/cli.js`, fonctionne dans tous les cas. | élevée | `tasks.md` : la première tâche rétablit la commande — à la personne de purger ce cache, il est hors du dépôt — et, à défaut, nomme le chemin de repli. Aucune tâche ne dépend d'une commande non essayée. |
